function [img]=frft_conv(obj,dx,dfx,r,a,fe);   %用卷积法来算傅里叶变换传播  正传播
...
dy=dx;
dfy=dfx;
alpha1=dx*dfx/(r*fe*sina);
alpha2=dy*dfy/(r*fe*sina);
%alpha=alpha1+alpha2

for ii = 1:mm
    for jj = 1:nn
    pha_obj(ii,jj) = exp(1i*pi/r/fe/tana2*(dx^2*(ii-1-ceil(mm/2))^2 + dy^2*(jj-1-ceil(nn/2))^2)); % x^2 + y^2
    end
end
obj=obj.*pha_obj;
...
for ii=1:mm
    for jj=1:nn
        chrp1(ii,jj)=exp(-1i*pi*(alpha1*(ii-1-ceil(mm/2))^2+alpha2*(jj-1-ceil(nn/2))^2));
    end
end
obj=obj.*chrp1;
...
for ii=1:mm
    for jj=1:nn
        chrp2_temp(ii,jj)=exp(1i*pi*(alpha1*(ii-1-ceil(mm/2))^2+alpha2*(jj-1-ceil(nn/2))^2));
    end
end

mh=abs(floor(r*fe*sina*dx/(2*dfx^3)))
chrp2=zeros(mm,nn);
if mh<min(mm/2,nn/2)
...
for ii = 1:mm
    for jj = 1:nn
    pha_img(ii,jj) = exp(1i*pi/r/fe/tana2*(dfx^2*(ii-1-ceil(mm/2))^2 + dfy^2*(jj-1-ceil(nn/2))^2)); % x^2 + y^2
    end
end
img=img.*pha_img;