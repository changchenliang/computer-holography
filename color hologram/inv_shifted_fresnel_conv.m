function [img]=inv_shifted_fresnel_conv(obj,dx,dfx,r,z,origin1,origin2);   %用卷积法来算傅里叶变换传播  正传播
...
for ii=1:mm
    for jj=1:nn
        chrp2(ii,jj)=exp(-1i*pi/alpha1/mm/nn*((jj-1-ceil(nn/2))^2+(ii-1-ceil(mm/2))^2));
    end
end
%mh=abs(floor(r*z*dfx/(2*dx^3)))
%chrp2=zeros(mm,nn);
%if mh<min(mm/2,nn/2)
...
g=fftshift(fft2(fftshift(obj))).*chrp2;
img=fftshift(ifft2(fftshift(g)));
