clc;clear;
EI=double(rgb2gray(imread('pic\EI4800CD.jpg')));
[M,N]=size(EI);
%M=7200;N=7200;
D=60e-3;%直径
F=180e-3;%焦距  f-num=4
...
%H=fourierholo5(m,n,lens_d,lens_f,micr_N,r,EI);
%H=tilted_sum(m,n,lens_d,lens_f,micr_N,r,EI);
%H=frenelholo2(m,n,lens_d,lens_f,micr_N,r);
H=frenelholo1(m,n,lens_d,lens_f,micr_N,r,EI,z);
...
[du,img]=angular_spectrum(lens_d,r,H,z+0.06);
%  [dfx img ] = fresnel_cov(lens_d, r, H, z );
  figure; imshow(abs(img/max(max(abs(img)))));
